...
Author: slfan
Date: Nov 2021
refer to https://github.com/yanx27/Pointnet_Pointnet2_pytorch
"""
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='pcsod', help='model name [default: pcsod]')
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            seg_pred, seg_o = classifier(points)   # seg_pred, seg_o: B*4096*2
            seg_pred = seg_pred.contiguous().view(-1, NUM_CLASSES)   # seg_pred: (B*4096)*2

            target = target.view(-1, 1)[:, 0]   # target: B*4096
